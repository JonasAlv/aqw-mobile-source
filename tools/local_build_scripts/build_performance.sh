...
if [ -z "$LATEST_ASSET_URL" ]; then
    echo "Failed to fetch from GitHub API. Using cached version."
...
if [ "$LATEST_VERSION" != "$CACHED_VERSION" ] || [ ! -f "$CACHE_DIR/pristine_swfs/game.swf" ]; then
    ...
    echo "Up-to-date with upstream version ($CACHED_VERSION). Using cache."
fi

echo "Copying SWFs"
cp "$CACHE_DIR/pristine_swfs/"*.swf loader/gamefiles/

echo "Patching game.swf"
tools/local_build_scripts/patch_game.sh

echo "Patching game.swf for performance"
tools/local_build_scripts/patch_game_performance.sh
...
for render in auto gpu direct; do
  ...
  APK_NAME="out_performance/AQWPocket-Performance-armv8$suffix.apk"
