package ui {
	...
	public class Overlay extends MovieClip {
		...
		public function notification(message:String):void {
			this.notifications.addChild(new Notification(message));
			rearrangeNotifications();
		}

		public function rearrangeNotifications():void {
			if (this.notifications.numChildren > 0) {
				var firstNotif:Notification = Notification(this.notifications.getChildAt(0));
				this.notifications.x = stage.stageWidth - firstNotif.width - 10;
				this.notifications.y = 10;
			}
			for (var i:int = 0; i < this.notifications.numChildren; i++) {
				var notif:Notification = Notification(this.notifications.getChildAt(i));
				notif.y = i * (notif.height + 10);
			}
		}
